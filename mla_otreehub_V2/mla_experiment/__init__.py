...
def setup_player_round(player):
    ...
    # Copy participant-level variables into every round to simplify export/analysis.
    if 'cfc_score' in player.participant.vars:
        player.cfc_score = player.participant.vars['cfc_score']
    if 'loss_aversion_index' in player.participant.vars:
        player.loss_aversion_index = player.participant.vars['loss_aversion_index']
    if 'lottery_refusals' in player.participant.vars:
        player.lottery_refusals = player.participant.vars['lottery_refusals']
...
def compute_loss_aversion(player):
    choices = [player.lottery_1, player.lottery_2, player.lottery_3, player.lottery_4, player.lottery_5]
    refusals = sum(1 for choice in choices if choice == 'A')
    player.lottery_refusals = refusals
    player.loss_aversion_index = _round2(refusals / 5)
    player.participant.vars['lottery_refusals'] = player.lottery_refusals
    player.participant.vars['loss_aversion_index'] = player.loss_aversion_index
...
def compute_investment_outcome(player):
    ...
    # The block variables are filled when a feedback page is shown.
...
class LossAversion(Page):
    form_model = 'player'
    form_fields = ['lottery_1', 'lottery_2', 'lottery_3', 'lottery_4', 'lottery_5']
...
class Feedback(Page):
    ...
    def vars_for_template(player):
        ...
        fill_block_results(player)
        is_frequent = bool(player.frequent_feedback)
        # Visual salience must exist ONLY in the perceptive block of C/D.
        # A and B are always visually neutral, including B with frequent feedback.
